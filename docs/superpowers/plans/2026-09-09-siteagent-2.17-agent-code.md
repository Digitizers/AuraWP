...
- `create_file( $path, $content )`: stage (a non-`.php` temp name beside the target, exclusive create) → record `{ kind: 'file', target, existed: false, expected_sha256, staged }` → publish with `link()` (atomic, no-clobber). **No target path is ever visible without a record, and no partial content is ever visible at the target.** `link()` refused → `error: 'unsupported_filesystem'`, nothing at the target; target present at publish → `error: 'exists'`, staged file gone, record deleted (spec §5).
...
```php
	...
	public function test_create_file_refuses_an_existing_target_before_touching_anything(): void {
		...
		$this->assertSame( array(), glob( WP_CONTENT_DIR . '/.taken.php.aura-create-*' ), 'no staged file left' );
	}
	...
	public function test_target_appearing_between_stage_and_publish_is_exists_with_nothing_left_behind(): void {
		...
		$this->assertSame( array(), glob( WP_CONTENT_DIR . '/.race.php.aura-create-*' ) );
	}
	...
	public function test_record_persist_failure_leaves_no_staged_file_and_no_target(): void {
		...
		$this->assertFileDoesNotExist( $file );
		$this->assertSame( array(), glob( WP_CONTENT_DIR . '/.np.php.aura-create-*' ) );
	}
	...
	public function test_short_write_while_staging_leaves_no_staged_file_no_record_no_target(): void {
		...
		$snaps = new class extends Aura_Worker_Snapshots {
			protected function stage( $dir, $name, $content ) {
				...
				$tmp = $dir . '/.' . $name . '.aura-create-shortwrite';
		...
		$this->assertSame( array(), glob( WP_CONTENT_DIR . '/.short.php.aura-create-*' ) );
	}
	...
	public function test_link_refused_is_unsupported_filesystem_with_nothing_at_the_target(): void {
		...
		$this->assertFileDoesNotExist( $file );
		$this->assertSame( array(), $snaps->list_snapshots() );
		$this->assertSame( array(), glob( WP_CONTENT_DIR . '/.nolink.php.aura-create-*' ) );
	}
	...
	public function test_staged_name_is_never_a_php_name_and_lives_beside_the_target(): void {
		...
		$this->assertStringStartsWith( '.agent.php.aura-create-', basename( $snaps->seen ) );
		$this->assertStringNotContainsString( '.php', substr( basename( $snaps->seen ), -4 ), 'a stray staged file must never be served as PHP' );
	}

	public function test_a_stray_staged_file_older_than_an_hour_is_swept_by_the_next_create_in_that_directory(): void {
		$snaps = new Aura_Worker_Snapshots();
		$stray = WP_CONTENT_DIR . '/.old.php.aura-create-deadbeef';
		file_put_contents( $stray, 'partial' );
		touch( $stray, time() - 2 * HOUR_IN_SECONDS );
		$fresh = WP_CONTENT_DIR . '/.new.php.aura-create-cafe';
...
```php
	 ...
	 * the target. `xb` is an exclusive create: the name is ours or the call
	 * fails. A short write leaves nothing behind.
	 *
	...
	protected function stage( $dir, $name, $content ) {
		...
		$tmp = $dir . '/.' . $name . '.aura-create-' . $suffix;
...
- [ ] **Step 4: Implement the restore branch and the prune sweep**
