...
- Restore of an `existed: false` file record: missing → success; sha256 ≠ `expected_sha256` → `{ success: false, error: 'file_changed_since' }`; equal → removed (byte identity by owner ruling — no inode, no ctime) (spec §2, §5). **The path is CLAIMED before it is verified**: an atomic `rename()` of the target to an opaque `.aura-restore-<hex>` beside it takes the pathname out of the namespace, the claimed file is hashed, and only the claimed file is ever deleted — nothing that arrived at the path after the claim is touched (Codex #91 round-1 P1).
...
- **`total`, `published`, `drafts`, `agent_authored` are `null` when the CPT read hits its cap** (`CPT_CAP = 500` rows) **or failed** (`get_posts()` answers `[]` on a database error and only `$wpdb->last_error` says so). A partial count is not a count and an empty answer to a failed statement is not "no snippets"; `null` is the honest value and Aura already lowers, never hides, on it. `coverage` stays about the directory walk only (spec §3 pin).
...
- Produces (Power Pack 0.2.4 calls these — plan 3):
  ...
  - `restore( $id )` on such a record → `{ success: true }` or `{ success: false, error: 'file_changed_since' }` (with `moved_aside: <path>` when the changed file could not be put back because the path was re-taken in the window) or `{ success: false, error: <message> }`.
  - `protected function after_claim( $claim, $target )` — a no-op seam between the claim and the verification (tests model a concurrent writer there).
...
In `prune_older_than()`, insert into the `foreach ( $this->list_snapshots() as $rec )` body AFTER the existing multisite ownership check (`if ( $network && ! self::prunable_here( $rec, $main_site ) ) { continue; }`) and BEFORE the `door_kind` filter — so the ownership invariant that already governs every prune governs this sweep too (Codex #91 round-4 P2: the snapshot directory is shared by every blog on a network, and blog B must never delete blog A's staged bytes, least of all a slow create still in flight). Note the `door_kind` filter sits ABOVE the ownership check in the shipped method; move the ownership check above it (the two are independent — `prunable_here()` reads only `blog_id`), then insert this block between them:
...
`$GLOBALS['wpdb']` must be the `$wpdb` stub the suite already provides (the `_application_passwords` regex stub) — `sa_reset_state()` resets its `last_error`; if it does not, add `$GLOBALS['wpdb']->last_error = '';` there beside the new unset.
...
- **A heartbeat INSIDE the long phases** (Codex #91 round-2 P1): `private function heartbeat_during( $fence, $work )` hooks the upgrader's own sub-phase filters (`upgrader_pre_download`, `upgrader_source_selection`, `upgrader_pre_install`, `upgrader_post_install`) with a throttled `keep_self_update_claim()` (`static::LEASE_HEARTBEAT_SECONDS = 60`) for the duration of `$work()`, so a download, unpack or install that runs long keeps its lease alive; `'' === $fence` runs `$work()` plainly. Used by `update_plugin()`, `batch_update_one()`'s update phase, and `self_update_locked()`'s install (the rule, not the line). **Ruling:** `restore_plugin()` is one `ZipArchive::extractTo()` of SiteAgent's own ~1 MB zip — sub-second, no seam inside it, and an abort mid-extract would leave the directory half-restored, which is worse than the race. It stays an indivisible phase, renewed immediately before; a request that stalls ten minutes inside one extract is, by this lock's design, a dead holder. Same residual #78 round 25 accepted for the self-update's own `install()`. **Owner cap (2026-09-09, Codex #91 round 4):** the same holds for ANY single blocking upgrader call (`download_url()`, `unzip_file()`, `copy_dir()`) — PHP is single-threaded, so no callback runs while one blocking call is in progress, and a lease that cannot expire during such a call needs a holder-set expiry in the claim primitive itself, which the connect and the site claim share. That is SA#92, its own change; plan 2 ships what the primitive allows today (boundary renewals, in-phase heartbeat on the upgrader's filters, and a hard stop on the first failed check so an outlived request never restores or rolls back over its successor).
...
The existing `test_a_request_that_outlived_the_takeover_does_not_release_its_successors_claim` (~line 771) models the SAME seizure-during-install and asserts `$res['success']` is `true` — the pre-round-3 behaviour. Change its two result assertions to the new outcome and keep its claim assertion, which is the point of that test:
...
Expected: FAIL — `Cannot override private method` / `Call to undefined method before_guarded_restore` / the lost-lease entry reads `updated` / the seized-during-install self-update still answers `success: true`.
...
5. The heartbeat, beside `keep_self_update_claim()`:

```php
	/** Seconds between lease renewals inside one phase. `static::` so a test can zero it. */
...
Wrap the three update phases in it — the rule, not the line:

- `batch_update_one()`: `$update_result = $this->heartbeat_during( $fence, function () use ( $plugin_file ) { return $this->update_single_plugin( $plugin_file ); } );`
- `update_plugin()`: inside its `guarding_self` closure, `function ( $fence ) use ( $plugin_file ) { return $this->heartbeat_during( $fence, function () use ( $plugin_file ) { $skin = new Automatic_Upgrader_Skin(); $upgrader = new Plugin_Upgrader( $skin ); return $upgrader->upgrade( $plugin_file ); } ); }`
- `self_update_locked()`: `$result = $this->heartbeat_during( $fence, function () use ( $upgrader, $install_source ) { return $upgrader->install( $install_source, array( 'overwrite_package' => true ) ); } );` — the download above it already sits between two renewals and `download_url()` fires no upgrader filter; leave it. **And the lease is CHECKED right after the install returns, before any result handling** (Codex #91 round-3 P1): a lost fence there means a successor self-update is running on these files now, so neither the failed-install restore nor the verdict-and-rollback below may run — either would overwrite the successor's work. Insert directly after the `$tmp` cleanup that follows the install:
...
and REPLACE the later renewal whose result the shipped code ignores — the two lines
...
with the result beside `self_update_claim_lost()`:
...
The bootstrap's `add_filter()` / `apply_filters()` / `remove_filter()` stubs (`tests/bootstrap.php` ~lines 1355–1460) already run callbacks in priority order, pass the value through, and unregister by identity (`$cb === $callback`, which holds for the same closure instance) — nothing to teach.

6. `restore_plugin_guarded()`:
...
```php
	public function restore_plugin_guarded( $rollback, $plugin_slug, $backup_path ) {
		...
		$result      = $this->guarding_self( $plugin_file, function ( $fence ) use ( $rollback, $plugin_slug, $backup_path, &$lost ) {
			...
			// halfway — so it is the one phase kept indivisible (ruling in this
...
**Ruling (plan author, 2026-09-09; the owner reviews it on this plan's PR):** SA#79 records that the self-update claim lives in the per-blog `wp_options` while `WP_PLUGIN_DIR` is shared network-wide, so on multisite two subsites can each take their own claim and update the same directory concurrently. Multisite is unsupported by this plugin (readme: "designed for single WordPress installations"), and moving the claim into `sitemeta` means changing a hardened claim primitive (`take_claim` / `refresh_claim` / `release_claim`, regex-matched `$wpdb` stubs) for a configuration nobody runs. The cheapest honest closure is to **refuse EVERY Aura-driven mutation of SiteAgent's own files on a multisite network** with one code — the self-update, AND the generic paths that reach the same directory under the same per-blog claim: `update_plugin()` for `SELF_PLUGIN_FILE`, the SiteAgent entry of `batch_update_plugins()` (reached by `/v2/update/batch` and by the `update_plugin_safely` tool), and `restore_plugin_guarded()` for the `digitizer-site-worker` slug (Codex #91 round-1 P1: guarding only `self_update()` left the race reachable through Aura on the other three). Other plugins' entries are untouched. Cost if wrong: a multisite site in the fleet that updated SiteAgent through Aura before now answers a refusal and needs a network-admin update; it never silently races. If the owner would rather keep multisite as-is, drop this task — nothing else depends on it.

**Files:**
- Modify: `digitizer-site-worker/includes/class-aura-worker-updater.php` (`self_update()` ~line 177 before `take_claim`; `guarding_self()` as left by Task 4; `batch_update_plugins()`)
- Test: `tests/unit/SelfUpdateRecoveryTest.php`

**Interfaces:**
- Consumes: Task 4's `guarding_self( $plugin_file, $work, &$busy )` and `self_update_busy()`.
- Produces: `private function self_mutation_refusal( $plugin_file )` → the refusal array `{ success: false, code: 'aura_self_update_multisite_unsupported', error: <sentence>, in_progress: false }` when `$plugin_file` is `SELF_PLUGIN_FILE` and `is_multisite()`, else `null`. `guarding_self()` gains a fourth by-ref parameter `&$refused` (the array, or `null`) and runs nothing when it is set; `self_update()` answers the same array before any claim, download, or write; a batch entry for SiteAgent answers `{ status: 'failed', detail: <the sentence> }`.
...
Run: `vendor/bin/phpunit --testdox --filter multisite`
Expected: FAIL — `Undefined array key "code"` / `success` is `true` / the batch entry reads `updated`.

- [ ] **Step 3: Implement**

One helper beside `self_update_busy()`:
...
`guarding_self()` (as Task 4 left it) gains the refusal as its first check and a fourth parameter:
...
Its three callers read it:

- `update_plugin()`: after the `guarding_self(…, $busy, $refused)` call, `if ( null !== $refused ) { return $refused; }` before the `$busy` check.
- `batch_update_plugins()`: `guarding_self( …, $busy, $refused )`; then `if ( null !== $refused ) { $entry = array( 'plugin' => $plugin_file, 'status' => 'failed', 'detail' => $refused['error'] ); } elseif ( $busy ) { …existing skipped entry… }`.
- `restore_plugin_guarded()`: `guarding_self( …, $busy, $refused )`; `if ( null !== $refused ) { return $refused; }` before the `$busy || $lost` check.
...
```bash
git add digitizer-site-worker/includes/class-aura-worker-updater.php tests/unit/SelfUpdateRecoveryTest.php
git commit -m "fix(updater): refuse every Aura-driven mutation of SiteAgent's own files on multisite — the per-blog lock cannot serialise a shared plugin dir (SA#79)"
```
...
```
- `get_posts()` honours an array `post_status` and `posts_per_page`, and runs `$GLOBALS['_sa_get_posts_effect']( $args )` first when set (a test models a failing statement by setting `$wpdb->last_error` there); `post_type_exists()` reads `$GLOBALS['_post_types']` (register one with `$GLOBALS['_post_types']['angie_snippet'] = true`). All since 2.17.0 (`audit_agent_code`).
```
...
- Spec §6 step 2 bundle: SA#83 → Task 1, SA#80 → Task 4, SA#79 → Task 5 (ruling stated; covers all four self-mutation paths), `blocked_result()` → Task 6, release → Task 7.
- Codex #91 round 4 (P1, owner cap): the lease residual — a single blocking upgrader call longer than the window — is named in Task 4 and filed as SA#92 (holder-set expiry in the claim primitive); not a plan-2 task.
- Codex #91 round 4: the staged sweep in `prune_older_than()` runs only after the existing per-blog ownership check (test: blog 2 leaves blog 1's staged bytes; blog 1 sweeps them).
- Codex #91 round 3: `self_update_locked()` checks the lease right after the install and again before the probe; a loss on either stops with `self_update_claim_lost_after_install()` — no restore, no verdict, no rollback over the successor's files; the pre-existing outlived-request test flips to that outcome.
- Codex #91 round 2: `heartbeat_during()` on every upgrader phase (single, batch, self-update install) with `extractTo` ruled indivisible; `snippet_rows()` reads `$wpdb->last_error` and answers `null` — every CPT-derived figure null, the walk still answers.
- Codex #91 round 1: the `existed: false` restore claims the path by atomic rename before verifying (three tests: newcomer after the claim untouched; changed file put back; changed file kept aside when the path was re-taken) — `.aura-restore-*` is never swept.
