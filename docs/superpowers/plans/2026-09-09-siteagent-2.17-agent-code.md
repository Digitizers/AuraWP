...
- `self_update_locked()`: `$result = $this->heartbeat_during( $fence, function () use ( $upgrader, $install_source ) { return $upgrader->install( $install_source, array( 'overwrite_package' => true ) ); } );` — the download above it already sits between two renewals and `download_url()` fires no upgrader filter; leave it.
